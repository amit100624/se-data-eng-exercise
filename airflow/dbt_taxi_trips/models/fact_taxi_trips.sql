...
WITH FACT_TAXI_TRIPS AS (
    SELECT
        ...
        {{ dbt_utils.generate_surrogate_key(['DATE(TPEP_PICKUP_DATETIME)::TIMESTAMP_NTZ']) }} AS PICKUP_DATE_ID,
        {{ dbt_utils.generate_surrogate_key(['DATE(TPEP_DROPOFF_DATETIME)::TIMESTAMP_NTZ']) }} AS DROPOFF_DATE_ID,
