...
WITH STG_SOURCE AS (
    SELECT
        ...
        PASSENGER_COUNT::FLOAT AS PASSENGER_COUNT,
        ...
        COALESCE(CREATED_TIMESTAMP, CURRENT_TIMESTAMP)::TIMESTAMP_NTZ(9) AS CREATED_TIMESTAMP
    FROM {{ source('taxi_trips', 'taxi_trips_raw') }}
),

STG_SOURCE_DEDUPLICATED AS (
    {{ dbt_utils.deduplicate(
         relation = 'STG_SOURCE',
         partition_by = 'VENDOR_ID, TPEP_PICKUP_DATETIME, PICKUP_LONGITUDE, PICKUP_LATITUDE',
         order_by = 'CREATED_TIMESTAMP DESC'
         )
    }}
),

STG_WITH_MANDATORY_FIELDS AS (
    SELECT * FROM STG_SOURCE_DEDUPLICATED
    ...
    SELECT
        ...
        CASE
            WHEN PASSENGER_COUNT = 0
                THEN (SELECT AVG_PASSENGER_COUNT FROM RAW_SOURCE_AGGREGATES)
        ...
        TRIP_SPEED_MPH,
        CREATED_TIMESTAMP
    FROM STG_WITH_MANDATORY_FIELDS, RAW_SOURCE_AGGREGATES
)
